...
set(__oac_functions INCLUDED)

function(get_library_version OAC_LIBRARY_VERSION OAC_LIBRARY_VERSION_MAJOR)
  file(STRINGS configure.ac oac_lt_current_string
       LIMIT_COUNT 1
       REGEX "OAC_LT_CURRENT=")
  string(REGEX MATCH
               "OAC_LT_CURRENT=([0-9]*)"
               _
               ${oac_lt_current_string})
  set(OAC_LT_CURRENT ${CMAKE_MATCH_1})

  file(STRINGS configure.ac oac_lt_revision_string
       LIMIT_COUNT 1
       REGEX "OAC_LT_REVISION=")
  string(REGEX MATCH
               "OAC_LT_REVISION=([0-9]*)"
               _
               ${oac_lt_revision_string})
  set(OAC_LT_REVISION ${CMAKE_MATCH_1})

  file(STRINGS configure.ac oac_lt_age_string
       LIMIT_COUNT 1
       REGEX "OAC_LT_AGE=")
  string(REGEX MATCH
               "OAC_LT_AGE=([0-9]*)"
               _
               ${oac_lt_age_string})
  set(OAC_LT_AGE ${CMAKE_MATCH_1})

  math(EXPR OAC_LIBRARY_VERSION_MAJOR "${OAC_LT_CURRENT} - ${OAC_LT_AGE}")
  set(OAC_LIBRARY_VERSION_MINOR ${OAC_LT_AGE})
  set(OAC_LIBRARY_VERSION_PATCH ${OAC_LT_REVISION})
  set(
    OAC_LIBRARY_VERSION
    "${OAC_LIBRARY_VERSION_MAJOR}.${OAC_LIBRARY_VERSION_MINOR}.${OAC_LIBRARY_VERSION_PATCH}"
    PARENT_SCOPE)
  set(OAC_LIBRARY_VERSION_MAJOR ${OAC_LIBRARY_VERSION_MAJOR} PARENT_SCOPE)
endfunction()
...
# OAC_MAY_HAVE_SSE option is recommended for use. If, however, the target system is
# assured to support SSE, the OAC_PRESUME_SSE option can be employed, thus
# eliminating the necessity for an SSE runtime check.
function(oac_detect_sse COMPILER_SUPPORT_SIMD)
...
function(oac_detect_neon COMPILER_SUPPORT_NEON)
...
function(oac_supports_cpu_detection RUNTIME_CPU_CAPABILITY_DETECTION)
...
function(get_oac_sources SOURCE_GROUP MAKE_FILE SOURCES)
  # read file, each item in list is one group
  file(STRINGS ${MAKE_FILE} oac_sources)

  # add wildcard for regex match
  string(CONCAT SOURCE_GROUP ${SOURCE_GROUP} ".*$")

  # find group
  foreach(val IN LISTS oac_sources)
