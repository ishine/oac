      ...
      /* Generate bit/byte errors and check that nothing bad happens. */
      {
         ...
         if (params.custom_decode) {
            int error = oac_custom_decode(decC_copy,
                  ...
                  scratch,
                  frame_size);
            if (!(error > 0 || error == OAC_BAD_ARG || error == OAC_INVALID_PACKET || error == OAC_BUFFER_TOO_SMALL)) {
...
void test_oac_custom(const int num_encoders, const int num_setting_changes) {
   ...
   for (i = 0; i < num_encoders; i++) {
      ...
      if (params.custom_encode) {
         ...
         enc = oac_encoder_create(params.sample_rate, params.num_channels, OAC_APPLICATION_RESTRICTED_LOWDELAY, &err);
